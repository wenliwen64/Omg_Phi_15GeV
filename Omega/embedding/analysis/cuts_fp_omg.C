...
int cuts_fp_omg(int nlist, int block,  string option){
    ...
    std::string filelist;
    if(option == "flat"){
	filelist = "../picodst_omg_flat.list"; 
    }
    else if(option == "exp"){
	filelist = "../picodst_omg_exp.list"; 
    }
    else{
	std::cout<<"BAD OPTION!!"<<std::endl;
    }
    //TChain * t = ChainThem(filelist.c_str(), "McV0PicoDst", nlist, block);
    TChain* chain = new TChain("McXiPicoDst");

    chain -> Add("../output_fp/*.large*.om.picodst.root");
    ...
    string filename = Dir+Name+stringify(nlist)+".coarse_ptbin_test.cuts.histo.root"; 
	...
	//hmMcXiPt[iCent] = new TH1F(hMcXiPtName.Data(), hMcXiPtTitle.Data(), 100, 0, 7);
	//hmRcXiPt[iCent] = new TH1F(hRcXiPtName.Data(), hRcXiPtTitle.Data(), 100, 0, 7);
...
*/
    ...
    string ofile_scale;
    if(option == "flat"){
	ofile = "./weight_omg_fp.txt";
	ofile_scale = "./weight_omg_fp_scale.txt";
    }
    else if(option == "exp"){
	ofile = "./weight_omg_exp.txt";
	ofile_scale = "./weight_omg_exp_scale.txt";
    }
    else{
	return -1;
    }
	...
	for(int j=0;j<kPtBin;j++){
	    float eff = hmRcXiPt[i] -> GetBinContent(j);
	    float err = hmRcXiPt[i] -> GetBinError(j);
	    if(eff != 0){
		oweight << i << " " << j << " " << eff << " " << err << std::endl; 
	    }
	}
	//Depending on previous result , begin with 2 and stop at 9 
	hmRcXiPt[i] -> Scale(1/pow(10, i));
	for(int j = 0; j < kPtBin; j++){
	    float eff = hmRcXiPt[i] -> GetBinContent(j);
	    float err = hmRcXiPt[i] -> GetBinError(j);
	    if(eff != 0){
		oweight_scale << i << " " << j << " " << eff << " " << err <<std::endl; 
	    }
	}
    }
