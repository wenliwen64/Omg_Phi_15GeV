int plot_phi(){
    ...
    int cent_bin[8] = {14, 24, 41, 66, 97, 141, 198, 234}; 
    int cent_no_bin[10] = {0, 5, 10, 20, 30, 40, 50, 60, 70, 100};
...
//===========QA Plots========================================
    ...
    while(fList.getline(linefromlist, 250)){
        file = new TFile(linefromlist, "read");
...
//==================Plot the Sig with Bg and Bg============================
	...
	for(int j = 0; j < 11; j++){
	    ...
	    TH1F* hist_bg = (TH1F*)HList.FindObject(hist_name_bg);
            ...
            sprintf(figure_name_1, "../../plots/phi/phipt%dcent%d.png", j+1, i+1);
            sprintf(figure_name_2, "../../plots/phi/phipt%dcent%d.eps", j+1, i+1);
...
//===============Plot the Invariant Mass Figures(After Subtraction)=================================
    float sig_count[9][11] = {};
    float sig_count_cal[9][11] = {};
    for(int i = 0; i < 9; i++){
	for(int j = 0; j < 11; j++){
	    char hist_name_sig[100];
	    char hist_name_bg[100];
	    ...
	    char figure_name_1[100];
	    char figure_name_2[100];
	    sprintf(figure_name_1, "../../plots/phi/pure_phipt%dcent%d.png", j+1, i+1);
	    sprintf(figure_name_2, "../../plots/phi/pure_phipt%dcent%d.eps", j+1, i+1);
	    new_canvas -> SaveAs(figure_name_1);
	    new_canvas -> SaveAs(figure_name_2);

            //=================Draw Integration Region==========================
            float x_low_line = 0.99; 
            float x_up_line = 1.05; 
	    ...
	    l2 -> SetLineColor(4);
            ...
            //=================Some Calculation==================================
            sig_count[i][j] = sig_func -> Integral(0.99, 1.05);  
            sig_count_cal[i][j] = par[2] / 0.0014;  
            //output_file << "signal counts by integrating sig_function is for cent" << i+1 << "pt " << j << " is " << sig_count[i][j] << std::endl;
            //output_file << "signal counts by area calculation is for cent" << i+1 << "pt " << j << " is " << par[2]/0.0014 << std::endl;
	    delete new_canvas;
	}
    }
//==============Plot Spectra========================================
...
//calculate the y coordinates of points
    ...
    for(int i = 0; i < 9; i++){
        ...
        for(int j = 0; j < 11; j++){
            ...
            output_file << i+1 << " " << j+1 << " " << y_pt_spectra_cal[i][j] << " " << sig_count_cal[i][j] << " " << nEvents_total << std::endl;
        }
    }
    ...
    cpt_phi -> SaveAs("phi_pt_spectra.eps");
    cpt_phi -> SaveAs("phi_pt_spectra.png");
    ...
    for(int i = 0; i < 9; i++){
        if(i == 8) continue;
        std::cout<<"graph no."<<i<<std::endl;
        TGraph* cur_g = new TGraph(11, x_pt_spectra, y_pt_spectra_cal[i]);
	cur_g -> SetMarkerSize(1.5);
	cur_g -> SetMarkerStyle(20);
	cur_g -> SetMarkerColor(2+i);
	cur_g -> SetMaximum(10.0);
    ...
    leg -> Draw("same");
    cpt_phi_cal -> SaveAs("phi_pt_spectra_cal.eps");
    cpt_phi_cal -> SaveAs("phi_pt_spectra_cal.png");
