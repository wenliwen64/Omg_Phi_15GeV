void embedding_qa(){
    //TFile* mc_file = new TFile("", "read");
    TFile* real_file = new TFile("0628_2015_omg.local_analysis.root", "read");
    ...
    //TH1F* hmV0Colinear[kCentBin][kPtBin]   new TH1F(hV0ColinearName.Data(), hV0ColinearTitle.Data(), 100, 0, 1);

    cout<<"happy mid"<<kPtBin<<endl;
    ...
    //TH1F* hmXiColinear[kCentBin][kPtBin]   new TH1F(hXiColinearName.Data(), hXiColinearTitle.Data(), 100, 0, 1);
    ...
    TLeaf* leaf_cenbin9 = fermidst->GetLeaf("cenbin9");
	...
	int nxi = leaf_nxi->GetValue(0);
        ...
        for(int ixi = 0; ixi < nxi; ixi++){
            ...
            float v0declen = leaf_v0declen->GetValue(v0declen);
            float v0mass = leaf_v0mass->GetValue(v0mass);
            float bachdca = leaf_bachdca->GetValue(bachdca);
            float dcabachtov0 = leaf_dcabachtov0->GetValue(dcabachtov0);
            float xipt = leaf_xipt->GetValue(xipt);
            float xidca = leaf_xidca->GetValue(xidca); 
            float xideclen = leaf_xideclen->GetValue(xideclen);
            float xirapidity = leaf_xirapidity->GetValue(xirapidity);
            float ximass = leaf_ximass->GetValue(ximass);
            float xisinth = leaf_xisinth->GetValue(xisinth);

            hDau1nHits[centbin][ptbin]->Fill(dau1nhits); 
            hDau2nHits[centbin][ptbin]->Fill(dau2nhits); 
            hBachnHits[centbin][ptbin]->Fill(bachnhits); 
            hDau1Pt[centbin][ptbin]->Fill(dau1pt);
            hDau2Pt[centbin][ptbin]->Fill(dau2pt);
            hBachPt[centbin][ptbin]->Fill(bachpt);
            hDau1Dca[centbin][ptbin]->Fill(dau1dca);
            hDau2Dca[centbin][ptbin]->Fill(dau2dca);
            hDca1to2[centbin][ptbin]->Fill(dca1to2);
            hV0Dca[centbin][ptbin]->Fill(v0dca);
            hV0DecLen[centbin][ptbin]->Fill(v0declen);
            hV0Mass[centbin][ptbin]->Fill(v0mass);

            hBachDca[centbin][ptbin]->Fill(bachdca);
            hDcav0tobach[centbin][ptbin]->Fill(dcav0tobach);
            hXiDca[centbin][ptbin]->Fill(xidca);
            hXiDecLen[centbin][ptbin]->Fill(xideclen);
            hXiMass[centbin][ptbin]->Fill(ximass);
            hXiRapdity[centbin][ptbin]->Fill(xirapdity); 
            hXiSinth[centbin][ptbin]->Fill(xisinth);
	}
    }
